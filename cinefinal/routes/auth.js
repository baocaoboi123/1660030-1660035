...
//LOGIN
    ...
    router.get('/forgot',asyncHandler(async function(req, res) {//tham số id để biết record cần chỉnh
       
        res.render('auth/forgot', { layout: './layouts/sidebar'});
     
    
    }));
    router.post('/forgot',asyncHandler(async function(req, res, next) {
       
        const {key_active} = req.body;
        const found_USer  = await NguoiDungAdmin.findUserByEmail(key_active);

        if(found_USer){
            var matkhau = randomstring.generate(5);
            const matkhaumoi  = await NguoiDungAdmin.updateThongTinUserPass(found_USer.IDnguoidung, matkhau);

            const subject = 'MẬT KHẨU MỚI CHO TÀI KHOẢN';
           
            const output = `
                <h2 align='center'>THAY ĐỔI MẬT KHẨU</h2>
                <br>
                <br>
                Chào bạn !
                <br>
                Bạn vừa gửi yêu cầu thay đổi mật khẩu trên email: ${key_active}
                <br>
            ...
            const send_Contact = await Email.sendContact(key_active,subject, output);
...
//REGISTER
    router.get('/root/option/function/register', function(req, res, next)
    {
        
        res.render('auth/option/registerAdmin', { layout: './layouts/sidebar', message:''});
    });
    router.post('/root/option/function/register',asyncHandler( async function(req, res, next)
    {
        const {hoten,email, password,confirmpassword,  sodienthoai} = req.body;
   
        const found_user =await NguoiDungAdmin.findUserByEmail(email);
        if (found_user ){  
            res.render('auth/option/registerAdmin', { layout: './layouts/sidebar', message: 'That email is already in use'});
        }else if(password !== confirmpassword){
            res.render('auth/option/registerAdmin', { layout: './layouts/sidebar', message: 'Passwords do not match'});

        }
        else{
            const found_add_user = await NguoiDungAdmin.addUser1(email, password, hoten, sodienthoai,'admin');
...
//ACTIVE
    ...
    router.get('/admin/config',asyncHandler(async function(req, res, next)
    {
        ...
        res.render('auth/config', { layout: './layouts/sidebar', found_AllMovie, message: ''});
    }));
        ...
        if (req.body.check_option === '0'){
            ...
            if(found_checkTenPhim ){
               console.log('tim thay ten phim');
                ...
                res.redirect('auth/admin/config');
                // const found_AllMovie =await Phim.find_All();
                // res.render('auth/config', { layout: './layouts/sidebar', found_AllMovie, message: 'thatbaiaddphim'});
            }else{
                ...
                // const found_AllMovie =await Phim.find_All();
                // res.render('auth/config', { layout: './layouts/sidebar', found_AllMovie, message: 'thanhcongaddphim'});
            }
    ...
    router.post('/admin/config/cumrap',upload.single('myImage'),asyncHandler(async function(req, res, next) {
        const fileName = req.file != null ? req.file.filename : null;
        ...
        if (req.body.check_option === '0'){
            ...
            const foundCumRapByID = await CumRap.findCumRapByID(req.body.edit_idcumrap);
            ...
            if(!foundCumRapByID){
                ...
                if(fileName === null)
                {
                    const found_updatedCumRap = await CumRap.updateCumRapByIDcumrap( req.body.edit_idcumrap,req.body.edit_tencumrap, req.body.edit_diachi, req.body.edit_mapdiachi,'', '1' );
                    ...
                    const found_updatedCumRap = await CumRap.updateCumRapByIDcumrap( req.body.edit_idcumrap,req.body.edit_tencumrap, req.body.edit_diachi, req.body.edit_mapdiachi,`/images/movie/${fileName}`, '0' );

                }
                console.log('tim thay');
                // const found_updatedCumRap = await CumRap.updateCumRapByIDcumrap( req.body.edit_idcumrap,req.body.edit_tencumrap, req.body.edit_diachi, req.body.edit_mapdiachi );
            ...
            const found_RapByCumRapIDcumrap_before = await Rap.findRapByIDcumrap(req.body.idcumrap2);
            ...
            const found_deleteCumRapByIDcumrap = await CumRap.deleteCumRapByIDcumrap(req.body.idcumrap2);
            ...
            const foundCumRapByTencumRap = await CumRap.findCumRapByTencumrap(req.body.add_tencumrap);
            if (foundCumRapByTencumRap[0] !== undefined){
                ...
                console.log('them thanh cong');
                const found_createdCumRap = await CumRap.createdCumRap(req.body.add_tencumrap, req.body.add_diachi, req.body.add_mapdiachi,`/images/movie/${fileName}`);
